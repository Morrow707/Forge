...
export default function CoachProgramBuilder() {
  ...
  const { data: program, isLoading } = useQuery<any>({
    queryKey: ["/api/coach/programs", programId],
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/coach/programs/${programId}`);
      return res.json();
    },
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await apiRequest("PUT", `/api/coach/programs/${programId}`, payload);
      return res.json();
    },
    onSuccess: () => {
  ...
  return (
    <AppShell
      ...
      actions={
        <>
          <Button variant="outline" onClick={() => navigate("/coach/programs")}>
