 ...
 * responses: no invite on file is a repair, an expired invite is a resend,
 * and a live unclaimed invite is a chase. */
export default function AdminBlockedAthletesPage() {
  ...
  const expired = rows.filter((a) => a.inviteSentAt && a.inviteExpired);
  const waiting = rows.filter((a) => a.inviteSentAt && !a.inviteExpired);
        ...
        <>
          ...
          <div className="space-y-3">
            {/* Worst first: an account with no invite cannot clear itself, so
                ...
                waiting. */}
            {[...noInvite, ...expired, ...waiting].map((a) => {
              const s = state(a);
              return (
                <Card key={a.id}>
